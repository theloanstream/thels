...
contract ThelsStream is Ownable {
    ...
    mapping(address => mapping(address => uint256)) public availableSFTokenBalances;
    ...
    event StreamInitiated(address superToken, address sender, address receiver, int96 flowRate);
    ...
    function depositErc20(address _token, uint256 _amount) public returns (bool) {
        ...
        erc20TokenBalances[_token][msg.sender] = erc20TokenBalances[_token][msg.sender].add(_amount);
    ...
    function withdrawErc20(address _recipient, address _token, uint256 _amount) public returns (bool) {
        require(erc20TokenBalances[_token][msg.sender].sub(_amount) >= 0, "Not enough tokens to withdraw");
        erc20TokenBalances[_token][_recipient] = erc20TokenBalances[_token][_recipient].sub(_amount);
        IERC20(_token).transfer(_recipient, _amount);
        return true;
    }

    // get ERC 20 token balance of the owner
    function getErc20TokenBalance(address _tokenOwner, address _token) public view returns (uint256) {
        return erc20TokenBalances[_token][_tokenOwner];
    }

    // get superfluid token balance of the owner
    function getSFTokenBalance(address _tokenOwner, address _superfluidToken) public view returns (uint256) {
    ...
    // get info of stream 
    function getStream(address _superToken, address _sender, address _recipient) public view returns (Stream memory) {
        return streams[_superToken][_sender][_recipient];
    }

    // upgrade erc20 token to superfluid token
    function upgradeToken(address _token, address _superToken, uint256 _amount) public returns (bool) {
        require(erc20TokenBalances[_token][msg.sender].sub(_amount) >= 0, "Not enough tokens to upgrade");
        // update balances
        erc20TokenBalances[_token][msg.sender] = erc20TokenBalances[_token][msg.sender].sub(_amount);
        availableSFTokenBalances[_superToken][msg.sender] = availableSFTokenBalances[_superToken][msg.sender].add(_amount);
    ...
    function downgradeToken(address _token, address _superToken, uint256 _amount) public returns (bool) {
        require(availableSFTokenBalances[_superToken][msg.sender].sub(_amount) >= 0, "Not enough Superfluid Tokens to downgrade");
        // update balances
        erc20TokenBalances[_token][msg.sender] = erc20TokenBalances[_token][msg.sender].add(_amount);
        availableSFTokenBalances[_superToken][msg.sender] = availableSFTokenBalances[_superToken][msg.sender].sub(_amount);
    ...
    function startStream(
        ...
        require(
            (availableSFTokenBalances[_superToken][msg.sender] - calculateStreamAmount(_flowRate, _streamingPeriod) - calculateBuffer(_flowRate, _bufferTime) - calculatePenalty(_flowRate)) > 0, 
        ...
        uint256 _streamAmount = calculateStreamAmount(_flowRate, _streamingPeriod);
        ...
        // update balance
        availableSFTokenBalances[_superToken][msg.sender] = availableSFTokenBalances[_superToken][msg.sender].sub(reservedTokens);
    ...
    function calculateStreamAmount (int96 _flowRate, uint256 _streamingPeriod) public view returns (uint256) {
        return (uint256(_flowRate) * _streamingPeriod);
    }

    function calculateBuffer(int96 _flowrate, uint256 _bufferTime) public view returns (uint256) {
        uint256 buffer;
    ...
    function calculatePenalty(int96 _flowrate) public view returns (uint256) {
        ...
        return calculatedPenalty < _minPenalty ? _minPenalty : calculatedPenalty;
    }

    function changePenaltyParameters(uint256 _newMinPenalty, uint256 _newStreamTine) public onlyOwner {
        _minPenalty = _newMinPenalty;
        _minStreamTime = _newStreamTine;
    }

    function resetStream(address _superToken, address _sender, address _receiver) internal {
    ...
    function deleteFlow(address _sender, address _receiver, address _superToken) public {
        
        require(streams[_superToken][_sender][_receiver].flowRate != 0, "There is no stream to be deleted");
        
        ISuperfluidToken superToken = ISuperfluidToken(_superToken);

        uint256 _endStreamTime = streams[_superToken][_sender][_receiver].end;
        uint256 _endBufferTime = streams[_superToken][_sender][_receiver].claimPenaltyAvailable;
        uint256 _paybackTotal = 0;

        if (block.timestamp <= _endBufferTime) {
            require(_sender == msg.sender, "Only owner can delete flow right now!");

            uint256 _paybackPenalty = streams[_superToken][_sender][_receiver].penaltyAmount;

            if (block.timestamp <= _endStreamTime) {
                uint256 _paybackStream = calculateRemainingStream(_sender, _receiver, _superToken);
                uint256 _paybackBuffer = streams[_superToken][_sender][_receiver].bufferAmount;
                _paybackTotal = _paybackPenalty + _paybackBuffer + _paybackStream;
            }

            if ((block.timestamp > _endStreamTime) && (block.timestamp <= _endBufferTime)) {
                uint256 _paybackBuffer = calculateRemainingBuffer(_sender, _receiver, _superToken);
                _paybackTotal = _paybackPenalty + _paybackBuffer;
            }

            // update balance and reset stream struct
            availableSFTokenBalances[_superToken][msg.sender] = availableSFTokenBalances[_superToken][msg.sender].add(_paybackTotal);
            ...
            _paybackTotal = calculateRemainingPenalty(_sender, _receiver, _superToken);
            // update balance and reset stream struct
            availableSFTokenBalances[_superToken][msg.sender] = availableSFTokenBalances[_superToken][msg.sender].add(_paybackTotal);
            resetStream(_superToken, _sender, _receiver);
            // cancel flow
    ...
    function calculateRemainingStream(address _sender, address _receiver, address _superToken) internal returns (uint256) {
        ...
        uint256 _flowRate = uint256(streams[_superToken][_sender][_receiver].flowRate);
    ...
    function calculateRemainingBuffer(address _sender, address _receiver, address _superToken) internal returns (uint256) {
        uint256 _timeNow = block.timestamp;
        uint256 _endTime = streams[_superToken][_sender][_receiver].end;
        uint256 _flowRate = uint256(streams[_superToken][_sender][_receiver].flowRate);
    ...
    function calculateRemainingPenalty(address _sender, address _receiver, address _superToken) internal returns (uint256) {
        uint256 _penalty = streams[_superToken][_sender][_receiver].penaltyAmount;
        uint256 _flowRate = uint256(streams[_superToken][_sender][_receiver].flowRate);
        uint256 _endBufferTime = streams[_superToken][_sender][_receiver].claimPenaltyAvailable;
